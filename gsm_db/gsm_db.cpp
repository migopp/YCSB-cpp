...
std::shared_ptr<std::string> expunged =
    std::make_shared<std::string>("__EXPUNGED__");
...
bool makeNullptrIfExpunged(std::shared_ptr<mapEntry> entry) {
    return entry->dataPointer.compare_exchange_strong(expunged, nullptr);
}

// return false if it was expunged
bool swapEntryValueIfEntryIsNotExpunged(std::shared_ptr<mapEntry> entry,
                                        std::shared_ptr<std::string> newValue) {
    while (true) {

        std::shared_ptr<std::string> currentValue = entry->dataPointer.load();
        if (currentValue == expunged) {
            return false;
        }

        // Only replace if it still has current value - we don't want to
        // overwrite an expunged. That would be painful and sad and I would cry
        // just a little
        if (entry->dataPointer.compare_exchange_strong(currentValue,
                                                       newValue)) {
            return true;
        }
    }
...
void deleteEntry(std::shared_ptr<mapEntry> entry) {
    while (true) {

        std::shared_ptr<std::string> currentValue = entry->dataPointer.load();
        if (currentValue == nullptr || currentValue == expunged) {
            // already deleted or expunged. No need to deal with this!
            return;
        }
        ...
        // overwrite an expunged. That would be painful and sad and I would cry
        // just a little
        if (entry->dataPointer.compare_exchange_strong(currentValue, nullptr)) {
            return;
        }
    }
}

DB::Status gsm_db::Read(const std::string &, const std::string &key,
...
DB::Status gsm_db::Insert(const std::string &, const std::string &key,
...
DB::Status gsm_db::Update(const std::string &, const std::string &key,
                          std::vector<Field> &values) {
...
DB::Status gsm_db::Delete(const std::string &, const std::string &key) {
    readOnlyMapEntryReturn entryFoundFromReadOnly =
        getEntryFromReadOnlyMap(key);
    if (entryFoundFromReadOnly.entry != nullptr) {
