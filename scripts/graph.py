...
def axis_names(task):
    ...
    if metric == "runtime":
        ...
        units = "op/sec"
    y = f"{stage}: {metric} ({units})"
    return (x, y)

def title(task):
    if task.workload == "a":
        return "50% Reads, 50% Updates"
    elif task.workload == "b":
        return "95% Reads, 5% Updates"
    elif task.workload == "c":
        return "100% Reads"
    elif task.workload == "d":
        return "95% Reads, 5% Inserts"
    elif task.workload == "e":
        return "95% Scans, 5% Inserts"
    elif task.workload == "f":
